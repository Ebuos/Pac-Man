...
@onready var inky_scatter_target: Area2D = $"../InkyScatterTarget"
...
@onready var inky_target: Marker2D = $"../../../Pac man/InkyTarget"
...
func _on_home_target_body_entered(body: Node2D) -> void:
	if body.is_in_group("Inky"):
		after_frightened()

func _on_timer_2_timeout() -> void:
	physics_enabled= true

func _on_inky_scatter_target_body_entered(body: Node2D) -> void:
	if body.is_in_group("Inky"):
		chase_mode()
